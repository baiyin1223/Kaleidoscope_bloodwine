...
public class VampirismCompat {
    ...
    private static class Impl {
        ...
        boolean isVampire(Player player) {
            return de.teamlapen.vampirism.api.VampirismAPI.factionPlayerHandler(player)
                    .isInFaction(de.teamlapen.vampirism.api.VReference.VAMPIRE_FACTION);
        }

        boolean isHunter(Player player) {
            return de.teamlapen.vampirism.api.VampirismAPI.factionPlayerHandler(player)
                    .isInFaction(de.teamlapen.vampirism.api.VReference.HUNTER_FACTION);
        }

        void onDrinkBloodWine(Player player, ItemStack stack, int brewLevel) {
            // 1. 恢复饮血值
            de.teamlapen.vampirism.api.entity.player.vampire.IVampirePlayer vampirePlayer =
                    de.teamlapen.vampirism.api.VampirismAPI.vampirePlayer(player);
        ...
        private void applySunscreenEffect(Player player, int brewLevel) {
            ...
            var sunscreenHolder = BuiltInRegistries.MOB_EFFECT.getHolder(
                    ResourceLocation.fromNamespaceAndPath(VAMPIRISM_MOD_ID, "sunscreen"));
            if (sunscreenHolder.isEmpty()) {
                return;
            }
            ...
            player.addEffect(new net.minecraft.world.effect.MobEffectInstance(sunscreenHolder.get(), duration * 20, amplifier, false, true));
        }
